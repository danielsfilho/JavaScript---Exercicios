...
formulario.onsubmit = e => {
    ...
    const nota = document.forms.formulario.inputScore.value
    filtrarAluno(nota)
}
...
function filtrarAluno(nota) {
  let htmlAlunos = alunos.reduce((acc, curr) => {
    if(curr.nota == nota) {
      acc += `
      <li>${curr.nome} ${curr.sobrenome} | Nota: ${curr.nota}. </li>
      `
    }
    ...
    if (htmlAlunos) {
      listStudent.innerHTML = htmlAlunos
      mensagem.innerText = ''
    }else {
    ...
    document.forms.formulario.inputScore.value = ''
  }
